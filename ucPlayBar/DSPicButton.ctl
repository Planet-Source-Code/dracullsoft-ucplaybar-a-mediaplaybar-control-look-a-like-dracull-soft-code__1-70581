VERSION 5.00
Begin VB.UserControl DSPicButton 
   AutoRedraw      =   -1  'True
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
...
' --------------------------------------------------------------------------------------
' --------------------------------------------------------------------------------------
' Copyright © 2000 Gene Martynov  (updated 2008 DracullSoft)
' Author or copyright holders can not be held responsible for anything.
' --------------------------------------------------------------------------------------
' Purpose:
' This button/checkbox control DOES NOT use Timer with following parameters you can set:
'  Image when mouse is over,Image when button is up,
'  Image when button is down,Image when button is disabled,
...
'  It works just like toolbar button except it does not have
'  a frame and you can specify all the images.
'  Based on original source code from:  Gene Martynov
'    http://www.Planet-Source-Code.com/vb/scripts/ShowCode.asp?txtCodeId=12218&lngWId=1
' ======================================================================================
Option Explicit


'__Event Declarations:
...
Public Enum eValUP
    ...
    abPressed = 1
End Enum
Private vval As eValUP

Public Enum eStyleButton
    abCheckButton = 1
    abStandardButton = 0
End Enum
Private sstyle As eStyleButton
...
Private m_ImageUp As Picture
Private m_Enabled As Boolean
Private m_ImageDown As Picture
Private m_ImageHot As Picture
Private m_ImageDisabled As Picture
Private m_BackStyle As eBackStyle
...
Public Property Get Style() As eStyleButton
    Style = sstyle
End Property

Public Property Let Style(ByVal New_Style As eStyleButton)
    sstyle = New_Style
...
Public Property Get Value() As eValUP
    Value = vval
End Property

Public Property Let Value(ByVal New_Value As eValUP)
    vval = New_Value
...
'   It detemines when mouse is over the control and when it goes out. This is made by using
'   mouse capturing. Whenever mouse is captured to a window, that window will get all the
'   mouse input.
'---------------------------------------------------------------------------------------
...
If Button = 1 Then
    ...
    If (wn <> UserControl.hWnd) And (wn <> 0) Then
        'This is supposed to happen when button is pressed and mouse is moved
...
If Button = 1 Then
    '__make sure we released mouse button inside the control
...
'__Initialize Properties for User Control
...
'__Load property values from storage
...
'Not used 'Call UserControl_Resize
